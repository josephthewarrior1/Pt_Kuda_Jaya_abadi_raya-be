...
const buildEmailHTML = ({ agentName, expiringSoon, expiredItems }) => {
  const typeLabel = 'Kendaraan';

  const renderSoonRow = (item) => {
    const daysLeft = getDaysLeft(item.carData?.dueDate);
    const title = `${item.carData?.ownerName || '-'} - ${item.carData?.carBrand || ''} ${item.carData?.carModel || ''}`.trim();
    const detail = `Plat: ${item.carData?.plateNumber || '-'}`;
    const dueStr = new Date(item.carData?.dueDate).toLocaleDateString('id-ID', {
      day: '2-digit',
      month: 'long',
      year: 'numeric',
    });
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:'Segoe UI',Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    <div style="background:#4F6EF7;border-radius:16px 16px 0 0;padding:28px 32px;">
    ...
    <div style="background:#fff;border-radius:0 0 16px 16px;padding:28px 32px;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
      ...
      <div style="margin-bottom:28px;">
        <h2 style="font-size:16px;color:#d97706;margin:0 0 12px;">
          Segera Jatuh Tempo (${expiringSoon.length})
        </h2>
...
const sendReminderEmail = async ({ to, agentName, expiringSoon, expiredItems, type = 'vehicle' }) => {
  const transporter = getTransporter();
  const typeLabel = 'Kendaraan';
  const totalIssues = expiringSoon.length + expiredItems.length;

  const subject = totalIssues > 0
    ? `[Reminder] ${totalIssues} Polis ${typeLabel} Perlu Perhatian - ${new Date().toLocaleDateString('id-ID')}`
    : `Semua Polis ${typeLabel} Aman - ${new Date().toLocaleDateString('id-ID')}`;

  const html = buildEmailHTML({ agentName, expiringSoon, expiredItems });
